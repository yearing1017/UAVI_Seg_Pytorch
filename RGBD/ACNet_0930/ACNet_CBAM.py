...
class ACNet(nn.Module):
    ...
    def encoder(self, rgb, depth):
        ...
        '''
        测试输出的shape
        ca_rgb_0: torch.Size([4, 64, 160, 160])
        ca_depth_0: torch.Size([4, 64, 160, 160])
        sa_rgb_0: torch.Size([4, 64, 160, 160])
        sa_depth_0: torch.Size([4, 64, 160, 160])
        m0: torch.Size([4, 64, 160, 160])
        '''


        ca_rgb_0 = self.c_atten_rgb_0(rgb).mul(rgb)
        #print(ca_rgb_0.shape)
        #print('c_atten_rgb_0:', self.c_atten_rgb_0(rgb).shape) # torch.Size([4, 64, 1, 1])
        ca_depth_0 = self.c_atten_depth_0(depth).mul(depth)
        #print(ca_depth_0.shape)
        #print('c_atten_depth_0:', self.c_atten_depth_0(depth).shape) # torch.Size([4, 64, 1, 1])
        # 经过通道注意力的数据再次经过空间注意力模块
        sa_rgb_0 = self.s_atten_rgb_0(ca_rgb_0).mul(ca_rgb_0)
        #print(sa_rgb_0.shape)
        #print('s_atten_rgb_0:', self.s_atten_rgb_0(ca_rgb_0).shape) # torch.Size([4, 1, 160, 160])
        sa_depth_0 = self.s_atten_depth_0(ca_depth_0).mul(ca_depth_0)
        #print(sa_depth_0.shape)
        #print('s_atten_depth_0:',self.s_atten_depth_0(ca_depth_0).shape) # torch.Size([4, 1, 160, 160]) 
        # 融合信息

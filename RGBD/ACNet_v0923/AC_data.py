...
class MyDataset(Dataset):
    ...
    def __len__(self):
        return len(os.listdir('data/dataset4/images_prepped_train'))

    def __getitem__(self, idx):
        image_name = os.listdir('data/dataset4/images_prepped_train')[idx]
        #print("image_name:"+"["+str(idx)+"]"+image_name)
        #image = Image.open('data/dataset1/images_prepped_train/' + image_name)
        image = cv2.imread('data/dataset4/images_prepped_train/' + image_name, 1)
        #image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
        dsm_data = cv2.imread('data/dataset4/dsm_prepped_train/' + image_name, 0)
        # 读label
        mask = cv2.imread('data/dataset4/anno_prepped_train/'+ image_name, 0)
...
if __name__ == "__main__":
    ...
    for index, (image, dsm_data, label) in enumerate(val_dataloader[0]):
        print(image.shape)
        print(dsm_data.shape)
        print(label.shape)
        print('=============')